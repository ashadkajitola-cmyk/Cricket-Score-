    ...
    <div id="dashboardSection" class="hidden">
        ...
        <div id="adminPanelSection" class="card hidden" style="border: 2px solid var(--accent); background: #fff;">
            ...
            <div id="mainAdminCodeBox" class="admin-section-box hidden">
                ...
                <button class="action-btn" id="sendPointsToUserBtn" style="background: #f57c00; margin-top:8px;">User ko Points Bhejein</button>
            </div>
...
<script>
    ...
    // Firebase Data Fetchers & Listeners
    function sendOTP() {
        ...
        db.ref('active_sessions/' + phone).once('value', (snapshot) => {
            ...
            // Fallback agar database rules ya network ki wajah se error aaye
            tempPhone = phone;
            generatedOTP = Math.floor(1000 + Math.random() * 9000).toString();
            alert("Aapka OTP hai: " + generatedOTP);
            document.getElementById('phoneStep').classList.add('hidden');
    ...
    function verifyOTP() {
        ...
        db.ref('active_sessions/' + tempPhone).set(myDeviceId).catch(err => console.log(err));

        db.ref('users/' + tempPhone).once('value', (snapshot) => {
            if(!snapshot.exists()) {
                let initialPoints = (tempPhone === ADMIN_NUMBER ? 15000 : 15);
                db.ref('users/' + tempPhone).set({ points: initialPoints, subscription: null, subExpiry: 0 });
            }
            localStorage.setItem('current_user', tempPhone);
            loadDashboard();
        }).catch((error) => {
            console.error("Verify Login Error:", error);
            // Fallback agar database block ho toh bhi login ho jaye
            localStorage.setItem('current_user', tempPhone);
            loadDashboard();
        });
    }
    ...
    function renderAdminMatchesList(matchesObj) {
        ...
        keys.forEach((id) => {
            ...
            html += `<div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #eee; padding:4px 0; font-size:12px;">
                ...
                <div>
                    <button onclick='editMatch("${id}", ${mJson})' style="background:#ff9800; color:white; border:none; padding:2px 6px; border-radius:3px; cursor:pointer; font-size:10px; width:auto; margin-right:4px;">Edit</button>
                    <button onclick="deleteMatch('${id}')" style="background:#dc3545; color:white; border:none; padding:2px 6px; border-radius:3px; cursor:pointer; font-size:10px; width:auto;">Delete</button>
                </div>
            </div>`;
        });
    ...
    function onAdminGroupSelected() {
        ...
        db.ref('points_tables/' + groupName).once('value', (snapshot) => {
            ...
            document.getElementById('ptTeam1Select').innerHTML = optionsHtml;
            document.getElementById('ptTeam2Select').innerHTML = optionsHtml;
        });
    }
    ...
    function savePointTableMatch() {
        ...
        if(!tableName || !team1 || !team2 || team1 === team2) {
            alert("Sahi Table aur Teams chunein!");
            return;
        }
        ...
        db.ref(`points_tables/${tableName}`).once('value', (snapshot) => {
            ...
            tables[team1].played += 1;
            tables[team2].played += 1;
            tables[team1].won += 1;
            tables[team1].points += 2;
            tables[team2].lost += 1;
        ...
        keys.forEach(tName => {
            ...
            teamKeys.forEach(t => {
                ...
                html += `<tr>
                    ...
                    <td>${d.played}</td><td>${d.won}</td><td>${d.lost}</td><td>${d.nr || 0}</td><td><b>${d.points}</b></td><td>${d.nrr || '0.000'}</td>
                </tr>`;
            });
            html += `</table></div>`;
        });
